...
export default function AlertsPage() {
    ...
    const handleFalsePositive = async (justification) => {
        ...
        try {
            ...
            setAlerts(prev => prev.map(a => a.id === fpModal.alertId ? {
                ...a,
                status: 'FALSE_POSITIVE', ...result
            } : a));
    ...
    return (
        <div className="h-full overflow-y-auto p-4 sm:p-5 lg:p-6">
                ...
                <div className="flex gap-2">
                    ...
                    <button onClick={() => setShowFilters(!showFilters)}
                        ...
                        {hasActiveFilters && <span
                            className="flex items-center justify-center w-5 h-5 rounded-full bg-brand-600 text-white text-xs">{filters.severity.length + filters.status.length + (filters.agentId ? 1 : 0)}</span>}
                    </button>
                </div>
            </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-6">
                        <div className="flex-1">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {SEVERITIES.map(s => <button key={s} onClick={() => toggleFilter('severity', s)}
                                                             className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors cursor-pointer ${filters.severity.includes(s) ? 'bg-brand-50 text-brand-600 border-brand-200' : 'text-gray-500 border-gray-200 hover:bg-gray-50'}`}>{s}</button>)}
                            </div>
                        </div>
                        <div className="flex-1">
                            <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Statut</p>
                            <div className="flex flex-wrap gap-2">
                                {STATUSES.map(s => <button key={s} onClick={() => toggleFilter('status', s)}
                                                           className={`px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors cursor-pointer ${filters.status.includes(s) ? 'bg-brand-50 text-brand-600 border-brand-200' : 'text-gray-500 border-gray-200 hover:bg-gray-50'}`}>{s}</button>)}
                            </div>
                        </div>
                    </div>
                    <div className="flex gap-4">
                        <div className="flex-1">
                            <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Date
                                début</p>
                            <input type="datetime-local" value={filters.from} onChange={e => {
                                setPage(0);
                        ...
                        <div className="flex-1">
                            ...
                            <input type="datetime-local" value={filters.to} onChange={e => {
                                setPage(0);
            ...
            <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                {loadingList ? (
                    <div className="flex items-center justify-center py-20"><Loader2
                        className="w-6 h-6 animate-spin text-brand-600"/></div>
                ) : alerts.length === 0 ? (
                    <div className="text-center py-20 text-gray-400"><Search
                        className="w-10 h-10 mx-auto mb-3 text-gray-300"/><p className="text-sm">Aucune alerte
                        trouvée</p></div>
                ) : (
                    <>
                            ...
                            <table className="w-full min-w-[920px] text-sm">
                                ...
                                {alerts.map(a => (
                                    <tr key={a.id} onClick={() => openDetail(a.id)}
                                        className={`cursor-pointer hover:bg-gray-50/50 transition-colors ${a._isNew ? 'animate-flash' : ''}`}>
                                        <td className="px-6 py-3.5 text-gray-500 whitespace-nowrap">{new Date(a.createdAt).toLocaleString('fr-FR', {
                                            day: '2-digit',
                                            month: '2-digit',
                                            hour: '2-digit',
                                            minute: '2-digit',
                                        ...
                                        <td className="px-6 py-3.5 text-center"><AlertSeverityBadge
                                            severity={a.severity}/></td>
                                        ...
                                        <td className="px-6 py-3.5 text-center"><AlertStatusBadge status={a.status}/>
                                        </td>
                                    </tr>
                                ))}
                                </tbody>
